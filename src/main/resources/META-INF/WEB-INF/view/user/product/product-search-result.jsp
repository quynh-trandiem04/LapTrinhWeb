											...
											<div class="col-xl-4 col-sm-6">
												<div class="wsus__product_item">
													...
													<a class="wsus__pro_link"
														href="/common/products/productdetail?id=${product.id}">
														<!-- Hình ảnh chính của sản phẩm --> <img
												...
												<div class="wsus__product_item wsus__list_view">
													...
													<div class="wsus__product_details">
														<a class="wsus__category"
															href="/common/products/productdetail?id=${product.id}">${product.category.name}</a>
														<p class="wsus__pro_rating">
															<c:forEach begin="1" end="5" varStatus="loop">
																<c:choose>
																	<c:when test="${loop.index <= product.rating}">
									...
									<ul class="pagination justify-content-center">
										...
										<c:choose>
											<c:when test="${action == 'filter'}">
												<c:if test="${productPage.number > 0}">
													<li class="page-item"><a class="page-link"
														href="<c:url value='/common/products/filter'/>?name=${searchName}&categoryName=<c:forEach items="${categoryName}" var="cat">${cat},</c:forEach>&brand=<c:forEach items="${brand}" var="br">${br},</c:forEach>&minPrice=${minPrice}&maxPrice=${maxPrice}&size=${productPage.size}&page=${productPage.number}">
											...
											<c:otherwise>
												<c:if test="${productPage.number > 0}">
													<li class="page-item"><a class="page-link"
														href="<c:url value='/common/products/search'/>?name=${searchName}&size=${productPage.size}&page=${productPage.number}">
															<i class="fas fa-chevron-left"></i>
													</a></li>
												</c:if>
												<c:if test="${productPage.number == 0}">
										...
										<c:forEach items="${pageNumbers}" var="pageNumber">
											<li
												...
												<c:choose>
													<c:when test="${action == 'filter'}">
														<a class="page-link"
															href="<c:url value='/common/products/filter'/>?name=${searchName}&categoryName=<c:forEach items="${categoryName}" var="cat">${cat},</c:forEach>&brand=<c:forEach items="${brand}" var="br">${br},</c:forEach>&minPrice=${minPrice}&maxPrice=${maxPrice}&size=${productPage.size}&page=${pageNumber}">
															${pageNumber} </a>
													</c:when>
													<c:otherwise>
														<a class="page-link"
															href="<c:url value='/common/products/search'/>?name=${searchName}&size=${productPage.size}&page=${pageNumber}">
															${pageNumber} </a>
													</c:otherwise>
												</c:choose>
											</li>
										...
										<c:choose>
											<c:when test="${action == 'filter'}">
												<c:if
													...
													<li class="page-item"><a class="page-link"
														href="<c:url value='/common/products/filter'/>?name=${searchName}&categoryName=<c:forEach items="${categoryName}" var="cat">${cat},</c:forEach>&brand=<c:forEach items="${brand}" var="br">${br},</c:forEach>&minPrice=${minPrice}&maxPrice=${maxPrice}&size=${productPage.size}&page=${productPage.number + 2}">
											...
											<c:otherwise>
												<c:if
													test="${productPage.number < productPage.totalPages - 1}">
													<li class="page-item"><a class="page-link"
														href="<c:url value='/common/products/search'/>?name=${searchName}&size=${productPage.size}&page=${productPage.number + 2}">
															<i class="fas fa-chevron-right"></i>
													</a></li>
												</c:if>
												<c:if
